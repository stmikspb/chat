...
class OAuthClient():
  ...
  def __init__(self, service_name, consumer_key, consumer_secret, request_url,
               access_url, handler, callback_url=None):
    """ Constructor."""
  ...
  def set_cookie(self, value, path='/'):
        self.handler.response.headers.add_header(
            ...
            ('oauth.%s' % self.service_name, value, path)
            )
        
  def get_cookie_username(self):
        return self.handler.request.cookies.get(
            'oauth.gdisb_username', ''
            )
    
  def set_cookie_username(self, value, path='/'):
        self.handler.response.headers.add_header(
            'Set-Cookie', 
            '%s=%s; path=%s; expires="Fri, 31-Dec-2021 23:59:59 GMT"' %
            ('oauth.gdisb_username', value, path)
            )
  ...
  def expire_cookie(self, path='/'):
        self.handler.response.headers.add_header(
            ...
            ('oauth.%s' % self.service_name, path)
            )
        self.handler.response.headers.add_header(
            'Set-Cookie', 
            '%s=; path=%s; expires="Fri, 31-Dec-1999 23:59:59 GMT"' %
            ('oauth.gdisb_username', path)
            )

class GDIClient(OAuthClient):
  def __init__(self, consumer_key, consumer_secret, callback_url,rhandler):
    """Constructor."""
    ...
    OAuthClient.__init__(self,
        ...
        consumer_key,
        consumer_secret,
        "http://gdi-accounts.appspot.com/oauth/request_token",
        "http://gdi-accounts.appspot.com/oauth/access_token",
        rhandler,
        callback_url)
  ...
  def get_authorization_url(self):
    ...
    return "http://gdi-accounts.appspot.com/oauth/authenticate?oauth_token=%s" % token
  ...
  def getUsername(self):
    return self.get_cookie_username()
